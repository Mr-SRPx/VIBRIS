...
SRP's Desktop Assistant V.I.B.R.I.S. Mark - V

Some modules are to be installed by writing these lines in PowerShell or Command Prompt-
-->   pip install -U speechRecognition, pyttsx3, wikipedia, wolframalpha, playsound, psutil
...
def me(res):
    if (res=="name"):
        ...
        print(dob)
def apologise():
    apl1="I am sorry sir! It is due to my technical disability. Hope you understand."
    apl2="Sir, I apologise for my technical error. But you only have to improve it."
    apl3="Please forgive me. I hope someday I'd be functioning like JARVIS!"
...
def speak(text):
    ...
    engine.setProperty("voice", schema[1].id)
...
def dt_time(func):
    ...
    if (func=="dtx"):
        ...
        tm_txt4="At this point of time it is " + time
...
def web():
    ...
    web4="sir, fill the URL to be visited\n>>> "
    web5="Provide the URL below\n>>> "
    WEB=random.choice([web1, web2, web3, web4, web5])
    speak(WEB)
    url=input(WEB)
    web5="Yes sir, opening..."
    web6="As you wish sir..."
...
def dice():
    ...
    dicenote2="Okay Sir rolling a dice..."
    dicenote3="Alright Sir dice's bein' rolled..."
    ...
    diceout2="Sir The outcome is" + diceout
    diceout3="Sir the result is" + diceout
...
def coin():
    ...
    coinnote2="Okay Sir tossing a coin..."
    coinnote3="Alright Sir coin is being tossed..."
    ...
    coinout2="Sir the outcome is" + coinout
    coinout3="Sir the result is" + coinout
...
def app_open(app_name):
    ...
    app_note2="Okay, opening it!"
    app_note3="Yes sir, I'm on it!"
    app_note4="Fire on the screen sir!"
    app_note5="Here you go sir!"
...
def app_close(app_name):
    app_note1="Okay sir! Closing it..."
    app_note2="Alright! Killing app..."
    app_note=random.choice([app_note1, app_note2])
    if (app_name=="vs_code"):
        applet="Code.exe"
...
def status(var):
    if (var=="cpu"):
        ...
        cpu_note1="Sir, the consumption of CPU is at", cpu_val, "%"
        cpu_note2="Sir, CPU usage level is at", cpu_val, "%"
        ...
        if (ram_percent <= 35.0):
            condition=" Therefore the performance is excellent."
        elif (ram_percent > 35.0) and (ram_percent <= 50.0):
            condition=" Therefore the performance is satisfactory."
        elif (ram_percent > 50.0) and (ram_percent <= 65.0):
            condition=" Therefore the performance is good."
        elif (ram_percent > 65.0) and (ram_percent < 80.0):
            condition="Therefore the performance is moderate."
        else:
            condition=" So the performance is critical."
        if (ram_percent >= 80.0):
            ram_lvl1="Sir, the consumption of RAM is at " + ram_prcnt + "%" + condition
            ram_lvl2="Sir, usage level of RAM is at " + ram_prcnt + "%" + condition
            ram_lvl=random.choice([ram_lvl1, ram_lvl2])
            speak(ram_lvl)
            print(ram_lvl)
            speak("Should I eject the interface to free the RAM sir ?")
            print("Should I eject the interface to free the RAM sir ?")
            ej_conf=listen()
...
def power(power_action):
    power_note1="Yes sir, doing it!"
    power_note2="As you wish sir!"
    power_note3="Okay sir, I'm on it!"
    power_note4="Alright, performing the protocol."
    power_note=random.choice([power_note1, power_note2, power_note3, power_note4])
    if (power_action == "reboot"):
        speak("Are you sure to reboot the system ?")
        print("Are you sure to reboot the system ?")
        reboot_conf=listen()
        ...
        speak("Are you sure to sign-out from the system ?")
        print("Are you sure to sign-out from the system ?")
        halt_conf=listen()
        if ("yes" in halt_conf) or ("yep" in halt_conf):
            speak(power_note)
            print(power_note)
...
def listen():
    ...
    return(query)
def wfalpha():
    wfa_inp1="What's your query Sir?"
    wfa_inp2="Sir, tell me your query!"
    wfa_inp3="For what query should I search, sir?"
    wfa_inp=random.choice([wfa_inp1, wfa_inp2, wfa_inp3])
    speak(wfa_inp)
    inp=listen()
    client=wolframalpha.Client("LE85EY-AKJVVJWX9H")
    port=client.query(inp)
    try:
        speak("Scanning in the servers of Wolfram Alpha")
        print("Scanning Wolfram Alpha Servers...")
        result=next(port.results).text
        wfa_conf_txt1="Found results, shall I speak it for you ?"
        wfa_conf_txt2="Got it, Need me to speak results sir ?"
        ...
        print(wfa_error)
        wfa_err1="Sorry sir, I amn't able to access Wolfram Alpha Servers."
        wfa_err2="I am sorry sir, the connection to the Wolfram Alpha Servers was unsuccessful."
        wfa_err=random.choice([wfa_err1, wfa_err2])
        print(wfa_err)
def off():
    off1="Okay, sir. I appreciate your time for me. Shutting myself down..."
    off2="Alright sir. As you wish! Terminating VIBRIS program..."
    off3="Thanks for your time sir. Deactivating VIBRIS program..."
    off4="As you wish sir! Ejecting myself..."
...
def detect():
    ...
    try:
        ...
        speak("Internet Connection Detected! We are online and ready.")
        print("Internet Connection Detected! We are online and ready")
    except requests.ConnectionError:
        speak("Sorry, the internet connection isn't reachable! System is offline!")
        print("Sorry, the internet connection isn't reachable! System is offline!")
def on():
    playsound.playsound("C:/VIBRIS/music/timer_beep.mp3")
    sleep(1)
    greet()
    speak("Welcome back to the desktop. Initialising speech drive and listening systems...")
    print("Welcome back to the desktop. Initialising speech drive and listening systems...")
    sleep(1.5)
    speak("Speech Drive and Listening systems checked!")
    print("Speech Drive and Listening systems checked!")
    speak("Engaging and checking control systems...")
    print("Engaging and checking control systems...")
    sleep(2)
    speak("Checked! Deploying Virtual UI...")
    ...
    detect()
def mail():
    speak("Whom to send the email sir ?")
    print("Whom to send the email sir ?")
    receive=listen()
    ...
    try:
        ...
        speak("I'm sorry sir, unable to send this mail at the moment!")
        print("I'm sorry sir, unable to send this mail at the moment!")
        print(mail_err)
def boredom():
    speak("To get rid of boredom what would you like to do - Play YouTube, Play Music, Play online games ?")
    print("To get rid of boredom what would you like to do- Play YouTube, Play Music, Play online games ?")
    act=listen()
    if ("youtube" in act):
        speak("Okay!")
        print("Okay!")
        ...
        if ("about" in cmd) and ("me" in cmd or "boss" in cmd or "srp" in cmd):
            ...
            try:
                ...
                try:
                    ...
                    speak("Sorry sir, I am unable to do or find that!")
                    print("Sorry sir, I am unable to do or find that!")
process_engine()
