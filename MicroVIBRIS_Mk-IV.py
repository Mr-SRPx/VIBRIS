"""
# SRP's Desktop Assistant V.I.B.R.I.S. Mark - IV

To intsall pyttsx3 module, run 'pip install -U pyttsx3==2.71' in Command Prompt or Power Shell.

To install speech recognition, run 'pip install speechRecognition' in PowerShell.
...
def speech_to_text():
    ...
    try:
        print("Recognising...")
        inp=rec.recognize_google(audio, language='en-in')
...
def speak(text):
    ...
    engine.runAndWait()
def about_VIBRIS():
    intro="I am VIBRIS, a micro-AI program made by Mr. SRP.\nMy script is made using Python. I am a text and voice-based virtual assistant version - \'Mark IV\'"
    speak(intro)
    print(intro)
...
def off():
    ...
    off2="Alright boss. As you wish! Terminating D.A.I.S.Y. program..."
    off3="Thanks for your time boss. Quitting D.A.I.S.Y. program..."
...
def AI_Engine():
    ...
    if (age > 103) :
        ...
        if (__name__ == "__main__"):
            while (True):
                plug=input("Click Enter to Speak\n(Type 'Guide' For Some Guide)")
                if (plug == ""):
                    inp=speech_to_text().lower()
                    if ("about" in inp) and ("me" in inp or "boss" in inp):
                        about_usr()
                    elif ("about" in inp or "who" in inp or "what" in inp) and ("you" in inp or "D.A.I.S.Y." in inp):
                        about_VIBRIS()
                    elif ("about" in inp) and ("me" in inp or "boss" in inp):
                        about_usr()
                    elif ("who" in inp) and (name in inp):
                        about_usr()
                    elif ("date" in inp) or ("day" in inp):
                        date()
                    elif ("timer" in inp):
                        timer()
                    elif ("time" in inp) or ("clock" in inp):
                        time()
                    elif ("guide" in inp) or ("help" in inp):
                        guider()
                    elif ("speak" in inp) or ("say" in inp):
                        say()
                    elif ("wish" in inp) or ("greet" in inp):
                        greet()
                    elif ("google" in inp):
                        sep_web("https://www.google.co.in/")
                    elif ("facebook" in inp) or ("fb" in inp):
                        sep_web("https://www.facebook.com/")
                    elif ("youtube" in inp) or ("yt" in inp):
                        sep_web("https://www.youtube.com/")
                    elif ("my site" in inp):
                        sep_web("http://code.maxez.ml")
                    elif ("surf" in inp) or ("browse" in inp) or ("internet" in inp):
                        web()
                    elif ("how" in inp or "ok" in inp) and ("you" in inp):
                        okay()
                    elif ("dice" in inp):
                        dice()
                    elif ("coin" in inp) or ("toss" in inp):
                        coin()
                    elif ("you" in inp) and ("stupid" in inp or "idiot" in inp or "dumb" in inp):
                        apologise()
                    elif (app_title in inp):
                        os.startfile(app_path)
                    elif ("bye" in inp) or ("shut" in inp and "down" in inp):
                        off()
                        break
                    else:
                        error()
